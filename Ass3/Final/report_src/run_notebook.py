"""Execute a notebook in place from a fresh kernel and report anything that would fail verify.py."""
import io
import sys
import time
...
BASE = ("f:/document/IFN_680_Advanced_Machine_Learning_and_Applications/"
        "weeks/week-05/project-3-aircraft-classification/notebook")
name = sys.argv[1]
path = f"{BASE}/{name}"

notebook = nbformat.read(io.open(path, encoding="utf-8"), as_version=4)
started = time.perf_counter()
# The training sweep is the long pole, so the timeout is generous. resources.metadata.path is what
# makes the kernel's working directory the notebook folder, which is what lets the supplied cells
# reach 'FGVCAircraft_Subset20/test' and 'best_model.pth' by their bare relative names.
...
for index, cell in enumerate(code):
    for output in cell.get("outputs", []):
        ...
        if output.get("output_type") in ("display_data", "execute_result") and \
            ...
            images += 1
    # A cell ending in a drawing call or a bare expression echoes the object it returns. Harmless,
    # but it is noise in a submitted notebook and no count of images or errors catches it.
...
print(f"  execution counts in order: {counts == list(range(1, len(code) + 1))}")
...
for index, text in stderr:
    print(f"    STDERR cell {index}: {text.strip()[:400]}")
