...
# The training notebooks need a GPU run to regenerate their stored outputs, so a code change there
# is a failure worth stopping for. main_report.ipynb is cheap to re-run, so a change there is routine.
COSTLY = ("cVAE_Baseline.ipynb", "cVAE_DiscriminatorLoss.ipynb", "DigitClassifier.ipynb")
...
for name in COSTLY:
    ...
    if not stored:
        raise SystemExit(
            ...
            f"'submission/project6_code.zip');\\\n"
