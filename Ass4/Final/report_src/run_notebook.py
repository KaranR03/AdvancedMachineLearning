"""Execute one case notebook in place from a fresh kernel and report what verify.py would fail.

The kernel's working directory is set to notebook/, which is what lets the notebook reach
Case1/Case1Dataset and Case1/resnet_frozen_best.pth by the same relative names a marker will use.

Run:  python tools/run_notebook.py case1.ipynb
"""
import io
import sys
import time
...
BASE = ("f:/document/IFN_680_Advanced_Machine_Learning_and_Applications/"
        "weeks/week-06/project-4-ai-auditing/notebook")
name = sys.argv[1]
path = f"{BASE}/{name}"
